...
export default function App() {
      ...
      fetch("/api/auth/login", {
        ...
        .then(data => {
          if (data.success && data.user) {
            setUser(data.user);
            // Fetch previous logs or starting chat
            fetch("/api/user/profile", {
              headers: { "x-user-id": data.user.id }
            })
              .then(pRes => pRes.json())
              .then(pData => {
                if (pData.chats && pData.chats.length > 0) {
                  setMessages(pData.chats);
                } else {
                  setDefaultGreeting(data.user);
                }
              })
              .catch(() => setDefaultGreeting(data.user));
          }
          setCheckedAuth(true);
        })
        .catch(() => {
  ...
  const handleLoginSuccess = (loggedInUser: UserProfile) => {
    ...
    fetch("/api/user/profile", {
      ...
      .catch(() => setDefaultGreeting(loggedInUser));
  };
